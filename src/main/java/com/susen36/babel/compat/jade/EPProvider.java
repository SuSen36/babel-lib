...
public enum EPProvider implements IEntityComponentProvider {
    ...
    public void appendTooltip(ITooltip tooltip, EntityAccessor accessor, IPluginConfig config) {
        ...
        if (entity instanceof LivingEntity living) {
            ...
            float threshold = BabelAttributes.getMaxElementalValue(living);
            if (threshold <= 0) return;
            AbstractEPCapability currentElement = ep.getCurrentElement();
            if (currentElement == null || currentElement.getValue() <= 0) return;
            tooltip.add(new EPElement(currentElement.getType(), currentElement.getValue(), threshold));
        }
    }
